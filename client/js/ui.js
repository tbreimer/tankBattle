...
function UI(){
  ...
  UI.prototype.tabBar = function(){
    numPlayers = Object.keys(world.players).length;

    width = Math.floor(windowWidth / 4);
    height = 40 * numPlayers;
    ...
    for (var z = 1; z < numPlayers; z++){
      uCtx.beginPath();
      uCtx.moveTo(x, y + (z * 40));
      uCtx.lineTo(x + width, y + (z * 40));
      uCtx.stroke();
    }
    ...
    for (var id in world.players) {
      ...
      // If player is playing, add him and his health to the players list, else add him to the spectators list

      if (user.type == "playing"){
        players.push(user.username);
        playersHealth.push(user.health);
      }else{
    ...
    for (var z = 0; z < players.length; z++){
      ...
      uCtx.fillText(username, x + 55, y + (z * 40) + 30);

      uCtx.strokeStyle = "green";
      uCtx.lineWidth = 5;

      uCtx.beginPath();
      uCtx.moveTo(x + 15, y + 16 + (z * 40));
      uCtx.lineTo(x + 25, y + 29 + (z * 40));
      uCtx.lineTo(x + 45, y + 9 + (z * 40));
      uCtx.stroke();    

      // Health
      healthWidth = Math.floor((width / 3) * (playersHealth[z] / player.maxHealth));
      healthHeight = 10;


      healthX = (x + width) - Math.floor(width / 3) - 15;
      healthY = y + 15 + (z * 40);

      uCtx.fillStyle = "red";
      uCtx.strokeStyle = "rgb(165, 0, 0)";

      uCtx.strokeRect(healthX, healthY, healthWidth, healthHeight);
      uCtx.fillRect(healthX, healthY, healthWidth, healthHeight);
    ...
    for (var z; z < numPlayers; z++){
      username = spectators[z - index];
      ...
      uCtx.beginPath();
      uCtx.moveTo(x + 18, y + 10 + (z * 40));
      uCtx.lineTo(x + 38, y + 30 + (z * 40));
      uCtx.stroke();   

      

      uCtx.beginPath();
      uCtx.moveTo(x + 18, y + 30 + (z * 40));
      uCtx.lineTo(x + 38, y + 10 + (z * 40));
      uCtx.stroke();   
    }
  }
  ...
  UI.prototype.lobby = function(){
    ...
    for (var id in world.players) {
      sortedPlayers.push(world.players[id]);
    }

    sortedPlayers.sort(function(a,b) {return (a.gameWins < b.gameWins) ? 1 : ((b.gameWins < a.gameWins) ? -1 : 0);} );


    for (var x = 0; x < sortedPlayers.length; x ++) {
      var user = sortedPlayers[x];
      ...
      if (uCtx.measureText(userText + stats).width < playerWidth - 20){
        uCtx.fillText(x + 1 + " " + userText + stats, userX, userY);
      }else{
        uCtx.fillText(x + 1 + " " + userText, userX, userY);
      }
    ...
    bouncesBoxX = Math.floor(70 + windowWidth / 2 + 60);
    bouncesBoxY = bulletsY + 25;
    bouncesBoxWidth = Math.floor((windowWidth - (70 + windowWidth / 2 + 90)) / 2) - 6;
    bouncesBoxHeight = 40;


    uCtx.font = "20px Arial";
    btText = "Bounces: " + world.bulletBounces;
    btWidth = uCtx.measureText(btText).width;
    btX = bouncesBoxX + (bouncesBoxWidth / 2) - (btWidth / 2);
    btY = bouncesBoxY + 27;

    // Select if host
    if (player.host == true){
      if (ui.mouseX > bouncesBoxX && ui.mouseX < bouncesBoxX + bouncesBoxWidth)
        if (ui.mouseY > bouncesBoxY && ui.mouseY < bouncesBoxY + bouncesBoxHeight){
          uCtx.fillStyle = "rgb(230, 230, 230)";
          uCtx.fillRect(bouncesBoxX, bouncesBoxY, bouncesBoxWidth, bouncesBoxHeight);
    ...
    uCtx.strokeStyle = 2;
    uCtx.strokeRect(bouncesBoxX, bouncesBoxY, bouncesBoxWidth, bouncesBoxHeight);
    uCtx.fillText(btText, btX, btY);

    // Speed box
    speedBoxWidth = Math.floor((windowWidth - (70 + windowWidth / 2 + 90)) / 2);
    speedBoxX = Math.floor(70 + windowWidth / 2 + 60) + bouncesBoxWidth + 6;
    speedBoxY = bulletsY + 25;
    speedBoxHeight = 40;

    uCtx.font = "20px Arial";
    stText = "Speed: " + world.bulletSpeed;
    stWidth = uCtx.measureText(stText).width;
    stX = speedBoxX + (speedBoxWidth / 2) - (stWidth / 2);
    stY = speedBoxY + 27;


    // Select if host
    if (player.host == true){
      if (ui.mouseX > speedBoxX && ui.mouseX < speedBoxX + speedBoxWidth)
        if (ui.mouseY > speedBoxY && ui.mouseY < speedBoxY + speedBoxHeight){
          uCtx.fillStyle = "rgb(230, 230, 230)";
          uCtx.fillRect(speedBoxX, speedBoxY, speedBoxWidth, speedBoxHeight);

          if (ui.click == true){
            world.cycleBulletSpeed();
          }
        }
    }
    ...
    uCtx.strokeStyle = 2;
    uCtx.strokeRect(speedBoxX, speedBoxY, speedBoxWidth, speedBoxHeight);

    uCtx.fillText(stText, stX, stY);
